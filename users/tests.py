...
class ImmediateEventTestCase(TestCase):
    def testCreatedEvent(self):
        self.user, self.token = User.objects.create_user_and_token(first_names="First 1", last_names="Last 1",
                                                                   login="Test")
        self.assertTrue(self.user.immediate_event != None)
